...
/**
 * Servlet implementation class CookieContoller
 */
@WebServlet(name ="coockieContoller", urlPatterns = { "/cookie" } )
public class CookieContoller extends HttpServlet {
	private static final long serialVersionUID = 1L;

	// WEB-INF 파일 내부에 작성하는 이유는 Servlet -> JSP로 작동되게끔 하기 위해서!! JSP로 바로 접근 불가능하도록!
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("CookieContoller::doGet() 호출");
		...
		visitCookie.setMaxAge(24 * 60 * 60); // 쿠키 만료 기간 설정. 단위:초(second) 쿠키 만료기간(maxAge)를 설정하지 않으면 브라우저가 닫힐 때 쿠키는 만료됨 
