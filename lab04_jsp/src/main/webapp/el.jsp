...
<body>
    ...
    <main>
        ...
        o. JSP 태그를 제외한 JSP 모든 코드 안에서는 언제든지 사용할 수 있음.
            ...
            - CSS 속성 값 설정
            - HTML 안에 <script></script> 태그에 포함된 JavaScript 코드 안에서
            - JSTL 안에서 사용 가능.
         --%>
         
         <p><%= 1 + 1 %> <%-- JSP expression(식) --%></p>
         <p>${1+1}</p> <%-- EL --%>
         
         <%-- 상태 저장에 사용되는 JSP 내장 객체
         o. pageContext: JSP 페이지가 유지되는 동안만 정보 저장.
         o. request: 요청 객체가 유지되는 동안 정보 저장.
         o. session: 세션이 유지되는는 동안 정보 저장
         ...
         EL &{ attribute }에서 상태 정보를 찾는 순서: 작은 크기에서 큰 크키로
            (1) ${ pageScope.attr }
            (2) ${ requestScope.attr }
            (3) ${ sessionScope.attr }
          ...
          <p> EL: id = ${ username } </p> <%-- ${ requestScope.username }와 동일 --%>
