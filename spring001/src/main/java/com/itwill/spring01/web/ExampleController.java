...
// 특정 클래스를 상속(extends)하거나, 특정 인터페이스를 구현(implements)할 필요 없는
// (상위 타입의 특정 메서드들을 반드시 재정의 할 필요가 없는) 평범한 자바 객체
// 스프링 MVC 프레임워크에서는 POJO로 작성된 클래스를 컨트롤러로 사용할 수 있음!
//	(비교) HttpServlet을 상속받는 클래스에서는 doGet(req, resp) 또는 doPost(req, resp)를 반드시 재정의(override)해야 웹 서비스(요청 처리)가 가능.

@Slf4j // -> private static final Logger log = LoggerFactory.getLogger(ExampleController.class); 코드를 자동으로 삽입.
@Controller // -> 디스패쳐 서블릿에세 컨트롤러 컴포넌트임을 알려줌.
// (1) servlet-context.xml 파일에서는 <context:component-san ... /> 설정
// (2) 컨트롤러 클래스에서는 @Controller 애너테이션을 사용.
//		-> 디스패쳐 서블릿이 컨트롤러 객체를 생성하고 관리할 수 있음.
public class ExampleController {
	
	@GetMapping("/") //GetMapping의 "/" 는 Contextroot를 뜻한다.
	public String home(Model model) {
		log.debug("home()");
		LocalDateTime now = LocalDateTime.now();
		model.addAttribute("now", now);
		// Model 객체는 컨트롤러에서 뷰로 데이터를 전달할 때 사용.
		// request.setAttribude(name, object)와 비슷한 기능.
		return "home";
		// -> 컨트롤러 메서드가 문자열을 리턴하면, 디스패쳐 서블릿이 뷰의 이름을 찾는 용도로 사용.
		// 디스패쳐 서블릿이 뷰 리졸버를 이용해서 /WEB-INF/views/returnValue.jsp 경로를 찾을 수 있음.
		// 접두사(WEB-INF/views/) 접미사(.jsp)가 붙는 이유는 servlet-context.xml에 <property> 속성으로 작성을 해뒀기 때문 
	}
	...
	@GetMapping("/ex1")  // 아규먼트로 ResquestParam을 지정해주는 경우 기본값을 설정할 수 있는 장점이 있지만 파라미터가 많아지면 아규먼트가 너무 길어진다.
	public void example1(@RequestParam(name="username") String username,
		...
		//  (1) request.getParameter("username"), Integer.ParseInt(request.getParameter("age")) 호출해서 요청 파라미터 값들을 읽고,
	...
	public String ex2(@ModelAttribute(name = "user") UserDto dto) {
		...
		// 디스패쳐 서블릿은 컨트롤러 메서드를 호출하기 위해서 UserDto 클래스 기본 생성자를 호출하고, '요청 파라미터 이름'으로 setter를 찾아서 호출한다.
	...
	@GetMapping("/rest1")
	@ResponseBody
	// 컨트롤러 메서드가 리턴하는 값이 뷰를 찾기 위한 문자열이 아니라, 클라이언트로 직접 응답되는 데이터.
	...
	public UserDto rest() {
		...
		// -> REST 컨트롤러가 리턴한 자바 객체를 jackson-databind 라이브러리에서 JSON(JavaScript Object Notation) 형식의 문자열로 변환하고
		//    클라이언트로 응답을 보냄. (pom.xml에 jackson-databind가 의존이 되어 있어야 함)
	}
