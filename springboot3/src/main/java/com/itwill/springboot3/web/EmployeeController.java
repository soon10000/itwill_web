...
@Controller @Slf4j @RequestMapping("/employee") @RequiredArgsConstructor
public class EmployeeController {
	...
	public void list(@RequestParam(name="p", defaultValue = "0") int pageNo, Model model) {
		...
		// SQL 쿼리문 중 offset ? : 몇번째 데이터 행부터 가지고 올 것인지
		// fetch first ? rows only : 몇개씩 데이터를 가지고 올것인지 (페이지에 보여지는 데이터 개수)
	...
	public String details(@PathVariable(name="id") Integer id, Model model) {
		log.info("dtails(id={})", id);
		Employee emp = empSvc.raed(id);
