...
public class CommentService {
	...
	public Page<Comment> readCommentList(Long postId, int pageNo){
		...
		log.info("data.number = {}, data.totlaPages = {}", data.getNumber(), data.getTotalPages());
	...
	public void update(CommentUpdateDto dto) {
		...
		// 아이디(PK)로 엔터티를 검색:
		Comment entity = commentRepo.findById(dto.getId()).get();
		
		// 검색된 엔터티의 필드를 업데이트
		entity.update(dto.getCtext());
		log.info("update(dto={})", entity);
		
		// commentRepo.save(entity)를 명시적으로 호출할 필요 없음. 단, Transactional 애너테이션이 있을때만!
	}
