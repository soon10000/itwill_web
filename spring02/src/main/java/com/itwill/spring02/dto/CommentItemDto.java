...
public class CommentItemDto {
	...
	public static CommentItemDto fromEntity(Comment comment) {
		return CommentItemDto.builder().id(comment.getId()).ctext(comment.getCtext()).username(comment.getUsername()).modifiedTime(Timestamp.valueOf(comment.getModifiedTime())).build();
	}
